"""Send the explicitly approved GN2d devnet pause transaction."""
...
def main() -> int:
    before = verify()
    if before["config"]["paused"]:
        print(json.dumps({"status": "already_paused", "sent": False}, sort_keys=True))
        return 0
    ...
    try:
        ...
        if status is None or status.err is not None:
            confirmation_error = f"pause_not_finalized:{status}"
    except Exception as exc:
        # Never resubmit after an ambiguous RPC confirmation failure. The
        # authoritative config state is checked below using a fresh read.
        confirmation_error = f"confirmation_rpc_error:{type(exc).__name__}"

    after = verify()
    if not after["config"]["paused"]:
        raise RuntimeError(confirmation_error or "pause_state_not_observed")
    print(
        json.dumps(
            {
                "status": (
                ...
                "signature": str(signature),
                "paused": True,
                "upgrade_ready": after["upgrade_ready"],
                "funds_transfer": False,
                "confirmation_warning": confirmation_error,
            },
            sort_keys=True,
        )
    )
